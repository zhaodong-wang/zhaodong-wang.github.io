...
Notes for Codex working in this repo. Covers current state, how it runs, and what to preserve vs. rebuild for the planned revamp.
...
- `.Codex/memory` is a symlink to the user's auto-memory directory; both `.Codex/` and `.DS_Store` are gitignored.
